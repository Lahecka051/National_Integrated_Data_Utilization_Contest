...
def simulate_slot(
    ...
    custom_end_hour: int | None = None,
) -> SlotRecommendation:
    """단일 슬롯 시뮬레이션. custom_start/end_hour로 사용자 시간 제약 반영 가능."""
...
def recommend_best_slots(
    errands: list[Errand],
    weeks: int = 4,
    time_constraint: dict | None = None,
) -> dict:
    """향후 N주간 모든 슬롯을 시뮬레이션하여 최적 3개 + 비추천 1개 반환.
    time_constraint: {"start_time": "14:00", "end_time": "16:00"} 형태로 사용자 시간 제약 전달.
    """
    today = datetime.now()
    slots = []
    ...
    for day_offset in range(1, weeks * 7 + 1):
        ...
        if target.weekday() >= 5:
            continue
        # 날짜 필터: start_date 이전 날짜는 건너뛰기
        if min_date and target < min_date:
            continue

        if time_constraint and custom_start is not None and custom_end is not None:
            # 시간 제약이 있으면 해당 시간대에 맞는 반차 유형만 시뮬레이션
            if custom_start < 12:
                half_day = HalfDayType.MORNING
            else:
                half_day = HalfDayType.AFTERNOON
            slot = simulate_slot(errands, target, half_day, custom_start, custom_end)
            slots.append(slot)
        else:
